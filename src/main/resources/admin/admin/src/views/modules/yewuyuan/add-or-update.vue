...
export default {
  data() {
	...
	shenfenzhenghao : false,
      ...
      rules: {
          yewuyuanzhanghao: [
                { required: true, message: '业务员账号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yewuyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yewuyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
